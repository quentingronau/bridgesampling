context('bridge_sampler.CmdStanFit works.')
...
test_that("stan_bridge_sampler", {
  ...
  if (require(cmdstanr)) {
    ...
    cmd_stan_model_H0 <- write_stan_file(stancodeH0)  |>
    ...
    cmdstan_fit_H0 <- cmd_stan_model_H0$sample(
      ...
      , iter_sampling = 3500, iter_warmup = 500, chains = 4
    ...
    cmdstan_fit_H0$init_model_methods()
     ...
     expect_is(
      H0_bridge_norm <- bridge_sampler(samples = cmdstan_fit_H0,
                                       method = "normal",
                                       silent = TRUE)
      , "bridge")
  
  expect_is(
      H0_bridge_norm_rep <-bridge_sampler(cmdstan_fit_H0,
                                          method = "normal",
                                          repetitions = 2, silent = TRUE)
      , "bridge_list")

    expect_is(
      H0_bridge_warp3 <- bridge_sampler(cmdstan_fit_H0, method = "warp3",
                                        silent = TRUE)
      , "bridge")

    expect_is(
      H0_bridge_warp3_rep <- bridge_sampler(cmdstan_fit_H0, method = "warp3", repetitions = 2, silent = TRUE)
      , "bridge_list")
