...
#' @param prior_prob numeric vector with prior model probabilities. If omitted, a uniform prior is used (i.e., all models are equally likely a priori). The default \code{NULL} correspond to equal prior model weights.
...
#'
#' @return For the default method and the method for \code{"bridge"} objects, a named numeric vector with posterior model probabilities (i.e., which sums to one).
#'
