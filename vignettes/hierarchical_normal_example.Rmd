---
title: "Hierarchical Normal Example"
author: "Quentin F. Gronau"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Hierarchical Normal Example}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

In this vignette, we explain how one can compute marginal likelihoods, Bayes factors, and posterior model probabilities using a simple hierarchical normal model.
The model that we will use assumes that each of the $n$ observations $y_i$ (where $i$ indexes the observation, $i = 1,2,...,n$) is normally distributed with corresponding mean $\theta_i$ and a common known variance $\sigma^2$: $y_i \sim \mathcal{N}(\theta_i, \sigma^2)$. Each $\theta_i$ is drawn from a normal group-level distribution with mean $\mu$ and variance $\tau^2$: $\theta_i \sim \mathcal{N}(\mu, \tau^2)$. For the group-level mean $\mu$, we use a normal prior distribution of the form $\mathcal{N}(\mu_0, \tau^2_0)$. For the group-level variance $\tau^2$, we use an inverse-gamma prior of the form $\text{Inv-Gamma}(\alpha, \beta)$. We will use `Jags` to fit the model which parametrizes the normal distribution in terms of the precision (i.e., one over variance). Consequently, we implement this inverse-gamma prior on $\tau^2$ by placing a gamma prior of the form $\text{Gamma}(\alpha, \beta)$ on the precision; we call this precision parameter `invTau2` in the code.
...
```
For obtaining the marginal likelihood, the `bridge_sampler` function expects a matrix with posterior samples (i.e., argument `samples`) where each row corresponds to one draw from the joint posterior. Consequently, we only extract the matrix with the posterior samples from the fitted `Jags` object as `samples_H0` and `samples_H1`.

The next step is to write the corresponding `log_posterior` function for both models.
...
```
Now we are ready for computing the log marginal likelihoods using the `bridge_sampler` function. We use `silent = TRUE` to suppress printing the number of iterations to the console:
```{r}
...
```
We see that both percentage errors are very small (i.e., smaller than 1%).

To compare the null model and the alternative model, we can compute the Bayes factor by using the `compute_bf` function.
...
```
In this case, the Bayes factor is close to one, indicating that there is not much evidence for either model. We can also compute posterior model probabilities by using the `compute_post_prob` function:
```{r}
